...
function [Elite_antlion_fitness,Elite_antlion_position,Convergence_curve]=ALO(N,Max_iter,lb,ub,dim,params,fobj)

% Initialize the positions of antlions and ants
antlion_position=initialization(N,dim,ub,lb,params);
ant_position=initialization(N,dim,ub,lb,params);
...
for i=1:size(antlion_position,1)
    [fitness,UpdatedPositions] = fobj(antlion_position(i,:),params);
    antlion_position(i,:) = UpdatedPositions;
    antlions_fitness(1,i)= fitness;
end
...
while Current_iter<Max_iter+1
    ...
    for i=1:size(ant_position,1)  
        ...
        [fitness,UpdatedPositions] = fobj(ant_position(i,:),params);
        ant_position(i,:) = UpdatedPositions;
